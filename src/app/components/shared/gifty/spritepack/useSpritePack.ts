...
export type Channel = "facebase" | "eyes" | "mouth" | "arms" | "body";
export type LoadedPack = { manifest: Manifest; sheets: Record<Channel, SheetImg> };
...
export function useSpritePack(): { hero: HeroState | null; pack: LoadedPack | null } {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const names: readonly Channel[] = ["facebase", "eyes", "mouth", "arms", "body"];
      const dims = await Promise.all(names.map((n) => loadImg(`${BASE}/${n}.webp`)));
      if (!alive) return;
      const sheets = {} as Record<Channel, SheetImg>;
