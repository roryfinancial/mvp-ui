...
export default function Auth({ onBack, onAuthComplete }: AuthProps) {
  ...
  const demoAccount = activeTab === "creator" ? DEMO_CREDENTIALS.creator : DEMO_CREDENTIALS.fan;
  ...
  async function handleSocialLogin() {
    // Social providers bypass the email/password flow — auto-login with demo.
    setLoading(true);
    const creds = DEMO_CREDENTIALS.creator;
    const result = await login(creds.email, creds.password);
    setLoading(false);
    if (result.ok) onAuthComplete?.(result.user.role === "creator" ? "creator" : "supporter");
  }
    ...
    <div className="min-h-screen bg-muted flex items-center justify-center p-4">
      ...
      >
        ...
        >
          ...
          >
            ...
            <div className="space-y-1 mb-3 font-mono text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-foreground">{demoAccount.email}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-subtle text-xs w-16">Password</span>
                <span className="text-foreground">{demoAccount.password}</span>
              </div>
            </div>
            <button
              onClick={autofill}
          ...
          >
            {[
              { label: "Continue with Google", icon: <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M12.24 10.29v3.1h5.21c-.22 1.4-1.12 3.12-3.83 3.12-2.3 0-4.18-1.9-4.18-4.25s1.88-4.25 4.18-4.25c1.19 0 2.08.51 2.57 1.05l2.76-2.67C19.22 4.19 16.22 3 12.24 3 7.37 3 3.4 6.7 3.4 12c0 5.3 3.97 9 8.84 9 4.88 0 8.35-3.59 8.35-8.61 0-.61-.06-1.19-.16-1.74h-8.2z" /></svg> },
              { label: "Continue with Twitch", icon: <svg className="w-5 h-5 text-[#9146FF]" viewBox="0 0 24 24" fill="currentColor"><path d="M11.571 5.094h1.714v4.714H11.571zm4.714 0h1.714v4.714H16.285zM8.571 0L3 5.571V19.8h4.714v4.714h4.714L19.8 19.8V0zm8.229 14.571h-2.857v2.857H13.44l-2.857 2.857v-2.857H6.285V3.857h10.515z" /></svg> },
              { label: "Continue with X", icon: <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M18.901 1.144h3.68l-8.157 9.066L24 22.846h-7.406l-5.698-6.321-6.191 6.321H0l8.665-9.617L.027 1.144h8.324l5.069 5.867L18.901 1.144Zm-.439 2.37L7.493 21.43h2.396l10.969-18.916h-2.396Z" /></svg> },
            ].map(({ label, icon }) => (
              <motion.button
                key={label}
